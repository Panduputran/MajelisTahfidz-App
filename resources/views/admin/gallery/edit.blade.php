...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-gray-200">
            <h1 class="text-2xl font-bold text-gray-900">Edit Gallery Item: {{ $gallery->title }}</h1>
        </div>
        ...
        <form action="{{ route('admin.gallery.update', $gallery) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            @if($gallery->image_path)
                <div>
                    ...
                    <div class="mt-1">
                        <img src="{{ asset('storage/' . $gallery->image_path) }}" alt="Current image" class="h-32 w-auto rounded-lg">
                    </div>
                </div>
            @endif
            ...
            <!-- Image -->
            <div>
                <label for="image_path" class="block text-sm font-medium text-gray-700">Change Image</label>
                <input type="file" name="image_path" id="image_path" accept="image/*"
                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                @error('image_path')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-sm text-gray-500">Leave empty to keep current image (JPEG, PNG, JPG, GIF - max 2MB)</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        <option value="Tahfidz" {{ old('category', $gallery->category) == 'Tahfidz' ? 'selected' : '' }}>Tahfidz</option>
